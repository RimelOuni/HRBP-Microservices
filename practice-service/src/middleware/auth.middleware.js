...
// URL interne réseau Docker — pour aller chercher les clés publiques
const KEYCLOAK_JWKS_URL = process.env.KEYCLOAK_JWKS_URL || "http://core-keycloak:8180";
// URL externe = celle annoncée par KC_HOSTNAME côté Keycloak — doit matcher EXACTEMENT le claim "iss"
...
const authMiddleware = (req, res, next) => {
  ...
  jwt.verify(
    ...
    (err, decoded) => {
      if (err) {
        console.error(">>> auth.middleware verify failed:", err.name, "-", err.message);
