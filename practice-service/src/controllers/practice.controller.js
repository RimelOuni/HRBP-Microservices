...
// ── Add collaborators to HRBP ────────────────────────────────────
exports.addCollaboratorsToHrbp = async (req, res) => {
  try {
    const { practiceId, hrbpId } = req.params;
    ...
    if (!Array.isArray(collaboratorIds) || collaboratorIds.length === 0)
      return res.status(400).json({ message: "collaboratorIds must be a non-empty array" });

    const practice = await Practice.findById(practiceId);
    if (!practice) return res.status(404).json({ message: "Practice not found" });
    if (!practice.hrbp.map(String).includes(String(hrbpId)))
      return res.status(400).json({ message: "HRBP is not assigned to this practice" });
    ...
    const result = await userService.bulkUpdateUsers(collaboratorIds, { ro_id: hrbpId }, token);
    res.json({ message: "Collaborators assigned to HRBP", modifiedCount: result.modifiedCount });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
};

// ── Remove collaborators from HRBP ───────────────────────────────
exports.removeCollaboratorsFromHrbp = async (req, res) => {
  try {
    const { collaboratorIds } = req.body;
    const token = getToken(req);
    ...
    const result = await userService.bulkUpdateUsers(collaboratorIds, { ro_id: null }, token);
    res.json({ message: "Collaborators removed from HRBP", modifiedCount: result.modifiedCount });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
};

// ── Add managers to HRBP ─────────────────────────────────────────
exports.addManagersToHrbp = async (req, res) => {
  try {
    const { practiceId, hrbpId } = req.params;
    ...
    if (!Array.isArray(managerIds) || managerIds.length === 0)
      return res.status(400).json({ message: "managerIds must be a non-empty array" });
    ...
    const result = await userService.bulkUpdateUsers(managerIds, { ro_id: hrbpId }, token);
    res.json({ message: "Managers assigned to HRBP", modifiedCount: result.modifiedCount });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
};

// ── Remove managers from HRBP ────────────────────────────────────
exports.removeManagersFromHrbp = async (req, res) => {
  try {
    const { managerIds } = req.body;
    const token = getToken(req);
    ...
    const result = await userService.bulkUpdateUsers(managerIds, { ro_id: null }, token);
    res.json({ message: "Managers removed from HRBP", modifiedCount: result.modifiedCount });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
};
