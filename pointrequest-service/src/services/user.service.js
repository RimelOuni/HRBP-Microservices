...
const getUserById = async (userId, token) => {
  ...
  try {
    ...
    console.error(`[pointrequest-service] user.service getUserById(${userId}) failed:`, err.message);
    return null;
  }
};
 ...
 * nécessaire pour créer/lister les demandes avec le bon requester.
 */
const getCurrentUserProfile = async (token) => {
  try {
    ...
    console.error("[pointrequest-service] user.service getCurrentUserProfile failed:", err.message);
    return null;
  }
};
